...
with tempfile.TemporaryDirectory() as raw:
        ...
        [
            {"timestamp": "2026-01-01T00:00:00Z", "type": "response_item", "payload": {
                "type": "message", "role": "user", "content": [{"type": "input_text", "text":
                    "You are authoring a new pull request. Work ONLY within the `Area` roadmap."}] }},
            {"timestamp": "2026-01-01T00:01:00Z", "type": "response_item", "payload": {
                "type": "function_call", "name": "exec_command", "call_id": "claim",
                "arguments": json.dumps({"cmd": 'claim.sh acquire "author/Area/item"'})}},
            {"timestamp": "2026-01-01T00:01:02Z", "type": "response_item", "payload": {
                "type": "function_call_output", "call_id": "claim", "output": "Wall time: 2 seconds"}},
            {"timestamp": "2026-01-01T00:02:00Z", "type": "response_item", "payload": {
                "type": "custom_tool_call", "name": "apply_patch", "call_id": "edit", "input": "patch"}},
            {"timestamp": "2026-01-01T00:02:01Z", "type": "response_item", "payload": {
                "type": "custom_tool_call_output", "call_id": "edit", "output": "Done"}},
            {"timestamp": "2026-01-01T00:03:00Z", "type": "response_item", "payload": {
                "type": "function_call", "name": "exec_command", "call_id": "build",
                "arguments": json.dumps({"cmd": "lake build"})}},
            {"timestamp": "2026-01-01T00:03:12Z", "type": "response_item", "payload": {
                "type": "function_call_output", "call_id": "build", "output": "Wall time: 12 seconds"}},
        ],
    )
    ...
    write_jsonl(
        ...
        [
            {"timestamp": "2026-01-01T00:00:00Z", "type": "response_item", "payload": {
                "type": "custom_tool_call", "name": "exec", "call_id": "search",
                "input": 'const r = await tools.exec_command({\n  cmd: "gh run view 1 --log | rg \\"lake build\\"",\n  yield_time_ms: 30000\n});'}},
            {"timestamp": "2026-01-01T00:00:01.300Z", "type": "response_item", "payload": {
                "type": "custom_tool_call_output", "call_id": "search", "output": "Script completed\nWall time 1.3 seconds"}},
            {"timestamp": "2026-01-01T00:00:01.300Z", "type": "event_msg", "payload": {
                "type": "item_completed", "item": {
                    "type": "CommandExecution", "command": ["bash", "-lc", 'gh run view 1 --log | rg "lake build"'],
                    "started_at_ms": 1767225600000, "completed_at_ms": 1767225601300,
                }}},
            {"timestamp": "2026-01-01T00:00:02Z", "type": "response_item", "payload": {
                "type": "custom_tool_call", "name": "exec", "call_id": "build",
                "input": 'const r = await tools.exec_command({\n  cmd: "set -e\\nlake build\\nlake exe axioms",\n  yield_time_ms: 30000\n});'}},
            {"timestamp": "2026-01-01T00:00:32Z", "type": "response_item", "payload": {
                "type": "custom_tool_call_output", "call_id": "build",
                "output": 'Script running with session ID 16678\nWall time 30.0 seconds'}},
            {"timestamp": "2026-01-01T00:00:33Z", "type": "response_item", "payload": {
                "type": "custom_tool_call", "name": "exec", "call_id": "poll",
                "input": "const r = await tools.write_stdin({session_id:16678,chars:\"\",yield_time_ms:30000});"}},
            {"timestamp": "2026-01-01T00:01:03Z", "type": "response_item", "payload": {
                "type": "custom_tool_call_output", "call_id": "poll", "output": "Script completed\nWall time 30.0 seconds"}},
            {"timestamp": "2026-01-01T00:01:04Z", "type": "response_item", "payload": {
                "type": "custom_tool_call", "name": "exec", "call_id": "patch",
                "input": 'const r = await tools.apply_patch("*** Begin Patch\\n*** End Patch");'}},
            {"timestamp": "2026-01-01T00:01:05Z", "type": "response_item", "payload": {
                "type": "custom_tool_call_output", "call_id": "patch", "output": "Done"}},
            {"timestamp": "2026-01-01T00:03:11Z", "type": "event_msg", "payload": {
                "type": "item_completed", "item": {
                    "type": "CommandExecution", "command": ["bash", "-lc", "set -e\nlake build\nlake exe axioms"],
                    "started_at_ms": 1767225602000, "completed_at_ms": 1767225791000,
                }}},
        ],
    )
...
infra = infrastructure_model(
    pr_builds=4, pr_runner_minutes=10, pr_runner_vcpus=8,
    main_runner_minutes=10, main_runner_vcpus=2,
    pr_runner_usd_minute=0.02, main_runner_usd_minute=0.01,
    cache_objects=1000, cache_gib=1,
...
assert abs(sum(
    component["equivalent_usd_per_merged_pr"]
    for component in infra["ci"]["decomposition"].values()
) - infra["ci"]["equivalent_usd_per_merged_pr"]) < 1e-12
renormalized = infrastructure_model(
    pr_builds=1, pr_runner_minutes=1, pr_runner_vcpus=1,
    main_runner_minutes=1, main_runner_vcpus=1,
    pr_runner_usd_minute=1, main_runner_usd_minute=1,
    cache_objects=0, cache_gib=0, retained_cache_gib=9.5,
    pr_component_shares=(2, 3, 5), main_component_shares=(1, 1, 2),
)
assert abs(sum(
    component["equivalent_usd_per_merged_pr"]
    for component in renormalized["ci"]["decomposition"].values()
) - 2) < 1e-12
...
try:
    infrastructure_model(
        ...
        main_runner_minutes=1, main_runner_vcpus=1,
        pr_runner_usd_minute=1, main_runner_usd_minute=1,
        cache_objects=0, cache_gib=0, retained_cache_gib=0,
...
expected_total = (
    scaling["integrated_linear_usd"] * 1_000_000
    + scaling["integrated_quadratic_usd"] * 1_000_000 ** 2
)
assert abs(projection["changed_loc"]["no_churn_cumulative_usd"] - expected_total) < 1e-9
assert abs(
    projection["net_retained_loc"]["cumulative_usd"]
    - projection["changed_loc"]["no_churn_cumulative_usd"] * 1.1
) < 1e-9
